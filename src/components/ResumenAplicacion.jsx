...
function ResumenAplicacion({
    ...
    const nombreTrabajo =
        trabajos?.find((t) => String(t.id_trabajo) === String(trabajoId))
            ?.tipo_trabajo || "";
    ...
    const superficieTotalAplicacion = useMemo(() => {
        return (tratamientos || []).reduce((acc, t) => {
            const supTrat = (t.lotes || []).reduce(
                (a, l) => a + toNum(l.superficie),
                0
            );
            return acc + supTrat;
        }, 0);
    ...
    // Totales por insumo a nivel aplicación
    ...
    const validarTrabajo = () => {
        ...
        if (!trabajoId) return "Debe seleccionar el tipo de trabajo.";
        if (!proveedorServiciosId)
            return "Debe seleccionar el proveedor de servicios.";

        if (!esLabranza && !proveedorInsumosId) {
    ...
    return (
        <div style={{ border: "2px solid #999", padding: "1rem", marginTop: "1rem" }}>
            <h3>Resumen de trabajos de {nombreTrabajo || "-"}</h3>
            ...
            >
                <div>
                    <strong>Orden:</strong> {ordenCarga || "-"}
                </div>
                <div>
                    <strong>Fecha:</strong> {fechaAplicacion || "-"}
                </div>
                <div>
                    <strong>Tambo:</strong> {tamboAplic?.nombre_tambo || "-"}
                </div>
                <div>
                    <strong>Tipo de trabajo:</strong> {nombreTrabajo || "-"}
                </div>
            </div>
            ...
            >
                <div>
                    <strong>Prov. servicios:</strong> {provServ?.nombre_proveedor || "-"}
                </div>

                {!esLabranza && (
                    <div>
                        <strong>Prov. insumos:</strong> {provIns?.nombre_proveedor || "-"}
                    </div>
                )}
            </div>
            ...
            {(tratamientos || []).map((t, idx) => {
                ...
                return (
                    ...
                    >
                        <h5>
                            {mayusculaInicial(nombreTrabajo)} {idx + 1} — {supTrat.toFixed(2)} ha
                        </h5>
                                            ...
                                            {(t.insumos || []).map((ins) => (
                                                <tr key={ins.id_insumo}>
                                                    ...
                                                    <td style={{ textAlign: "left" }}>
                                                        {toNum(ins.dosis).toFixed(3)}
                                                    </td>
                                                    <td>{ins.unidad_dosis || ""}</td>
                                                    <td style={{ textAlign: "left" }}>
                                                        {toNum(ins.cantidad_total).toFixed(2)}
                                                    </td>
            ...
            <div style={{ borderTop: "2px solid #999", marginTop: "1rem", paddingTop: "1rem" }}>
                <h4>TOTALES</h4>
                <h5>
                    <strong>Superficie total {nombreTrabajo}:</strong>{" "}
                    {superficieTotalAplicacion.toFixed(2)} ha
                </h5>
